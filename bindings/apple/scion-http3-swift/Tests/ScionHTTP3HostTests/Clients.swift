...
/// A client that trusts the server's own authority and knows the server's address.
func clientFor(
    ...
    config.dnsOverrides = [DnsOverride(host: "localhost", addresses: [server.endpoints.target])]
...
func requestTo(_ server: TestServer, _ path: String) -> HttpRequest {
    HttpRequest(url: server.url(path))
}

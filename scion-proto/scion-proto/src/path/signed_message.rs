...
impl SignedMessage {
    ...
    /// ## Usage Note:
    ///
    /// To infer the signature type `S` from the context, you can must explicitly specify the return
    /// type when calling this function, as shown below:
    ///  ```ignore
    /// # use scion_proto::path::signed_message::SignedMessage;
    /// let signature = SignedMessageSigner::sign_message_p256(...).unwrap()
    /// ```
    pub fn sign<'a>(

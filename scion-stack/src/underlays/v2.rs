...
//! Underlay stack v2. This will replace the current underlay stack implementations
...
use crate::{
    ...
    underlays::{
        ...
        v2::{
            ...
            udp::{UdpAsyncUdpUnderlaySocket, UdpUnderlaySocket},
        },
    },
};
...
impl V2UnderlayStack {
    /// Creates a new V2 underlay stack.
    pub fn new(
        preferred_underlay: PreferredUnderlay,
        underlay_discovery: Arc<dyn UnderlayDiscovery>,
        local_ip_resolver: Arc<dyn LocalIpResolver>,
    ...
    /// Selects the first underlay that matches the requested isd as.
    /// If available, the preferred underlay type is returned.
    ///
    /// XXX(uniquefine): We only use the ISD-AS to select the underlay, the bind address is ignored.
    /// In the unlikely case that user requests a specific IP a wildcard ISD-AS it could
    /// in theory happen that we select the wrong underlay.
...
impl DynUnderlayStack for V2UnderlayStack {
    fn bind_socket(
        &self,

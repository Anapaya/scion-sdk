...
/// encoded with the same content type. Use [`ConnectRpcAny`] for a service that
/// must also answer JSON.
pub struct ConnectRpc<T: ::buffa::Message>(pub T);

impl<T: ::buffa::Message> ConnectRpc<T> {
...
impl<T: ::buffa::Message + Debug> std::fmt::Debug for ConnectRpc<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("ConnectRpc").field(&self.0).finish()
    }
}

impl<T: ::buffa::Message> std::ops::Deref for ConnectRpc<T> {
    type Target = T;
...
impl<T: ::buffa::Message> IntoResponse for ConnectRpc<T> {
...
impl<T: ::buffa::Message> From<T> for ConnectRpc<T> {
...
/// `ConnectRpcAny` can only be used with types that are both buffa messages and
...
mod tests {
    ...
    fn into_response_sets_content_type_application_proto() {
        let response = ConnectRpc(Greeting { name: "ada".into() }).into_response();
    ...
    fn proto_extractor_rejects_a_json_content_type() {
        let result: Result<ConnectRpc<Greeting>, _> = extract(ConnectRpc::from_request(

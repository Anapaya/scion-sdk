...
impl CrpcClient {
    ...
    /// Unary RPC request with a [`prost`] message.
    ...
    {
        let body = self.post(path, req.encode_to_vec()).await?;
    ...
    /// Unary RPC request with a [`buffa`] message.
    ///
    /// Same call as [`unary_request`](Self::unary_request), for messages generated by
    /// `buffa-codegen` instead of `prost`. Only the encode and decode calls differ; the wire
    /// format is the same.
    pub async fn buffa_unary_request<Req, Res>(
        &self,
        path: &str,
        req: &Req,
    ) -> Result<Res, CrpcClientError>
    ...
    /// Posts `body` to `path` and returns the response body.
    ///
    /// Codec agnostic: the caller encodes the request and decodes the response.
    async fn post(&self, path: &str, body: Vec<u8>) -> Result<Bytes, CrpcClientError> {
        self.do_post(path, body)
            .instrument(tracing::info_span!("request", %path, id = rand::random::<u16>()))
            .await
    }

    async fn do_post(&self, path: &str, body: Vec<u8>) -> Result<Bytes, CrpcClientError> {

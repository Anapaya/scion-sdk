...
impl Config {
    ...
    /// (`ca_certs_file` / `ca_certs_dir` / `ca_certs_pem`) or a certificate
    /// verifier (`with_cert_verifier` / `with_platform_verifier`), peer
    /// verification, handshake and idle timeouts, and transport tuning. The application

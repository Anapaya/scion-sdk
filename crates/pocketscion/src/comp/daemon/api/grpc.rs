...
fn to_tonic_status(e: CrpcError) -> tonic::Status {
    match e.code {
        axum_connect_rpc::error::CrpcErrorCode::Canceled => tonic::Status::cancelled(e.message),
        axum_connect_rpc::error::CrpcErrorCode::Unknown => tonic::Status::unknown(e.message),
        axum_connect_rpc::error::CrpcErrorCode::InvalidArgument => {
            tonic::Status::invalid_argument(e.message)
        }
        axum_connect_rpc::error::CrpcErrorCode::DeadlineExceeded => {
            tonic::Status::deadline_exceeded(e.message)
        }
        axum_connect_rpc::error::CrpcErrorCode::NotFound => tonic::Status::not_found(e.message),
        axum_connect_rpc::error::CrpcErrorCode::AlreadyExists => {
            tonic::Status::already_exists(e.message)
        }
        axum_connect_rpc::error::CrpcErrorCode::PermissionDenied => {
            tonic::Status::permission_denied(e.message)
        }
        axum_connect_rpc::error::CrpcErrorCode::ResourceExhausted => {
            tonic::Status::resource_exhausted(e.message)
        }
        axum_connect_rpc::error::CrpcErrorCode::FailedPrecondition => {
            tonic::Status::failed_precondition(e.message)
        }
        axum_connect_rpc::error::CrpcErrorCode::Aborted => tonic::Status::aborted(e.message),
        axum_connect_rpc::error::CrpcErrorCode::OutOfRange => {
        ...
        axum_connect_rpc::error::CrpcErrorCode::Internal => tonic::Status::internal(e.message),
        axum_connect_rpc::error::CrpcErrorCode::Unavailable => {
            tonic::Status::unavailable(e.message)
        }
        axum_connect_rpc::error::CrpcErrorCode::DataLoss => tonic::Status::data_loss(e.message),
        axum_connect_rpc::error::CrpcErrorCode::Unauthenticated => {

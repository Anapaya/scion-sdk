...
use anapaya_ead_models::{EndhostApiDiscovery, EndhostApiGroup, EndhostApiInfo};
use anyhow::Context;
...
impl EndhostApiDiscoveryService {
    ...
    pub async fn start(
        ...
        if app_state.anapaya_ead_api(id).is_none() {
...
impl PocketScionState {
    ...
    pub fn add_anapaya_ead_api(&mut self) -> EndhostApiDiscoveryApiId {
        let mut sstate = self.write();
        let id = sstate.anapaya_ead_api.len().into();

        sstate.anapaya_ead_api.insert(id, EndhostApiDiscoveryState);

        id
    }

    /// Returns a map of all Endhost API Discovery APIs in the system state.
    pub(crate) fn anapaya_ead_apis(
        &self,
    ) -> BTreeMap<EndhostApiDiscoveryApiId, EndhostApiDiscoveryState> {
        self.read().anapaya_ead_api.clone()
    }

    /// Returns the state of the Endhost API Discovery API with the given id, if it exists.
    pub(crate) fn anapaya_ead_api(
        &self,
        id: EndhostApiDiscoveryApiId,
    ) -> Option<EndhostApiDiscoveryState> {
        self.read().anapaya_ead_api.get(&id).cloned()
    }
}
